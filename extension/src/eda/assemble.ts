...
import { resolveComponentUuids } from "./resolve-uuids";
...
async function createComponet(component: CircuitAssembly['components'][0], offset: Offset = { x: 0, y: 0 }) {
    ...
    if (!partUuid) {
        console.warn(`[AI Copilot] Skipping ${designator}: no LCSC UUID resolved`);
        return undefined;
    }
    ...
    if (partUuid === 'GND') {
        ...
        // Use the libraryUuid resolved during UUID resolution, or default to 'lcsc'
        const libUuid = (component as any)._libraryUuid || 'lcsc';
        comp = await placeComponent({
            libraryUuid: libUuid,
            uuid: partUuid
        }, { x, y, rotate: pos.rotate });
...
export async function assembleCircuit(circuit: CircuitAssembly) {
    ...
    // Resolve missing LCSC UUIDs via EasyEDA SDK before placing components
    try {
        await resolveComponentUuids(circuit.components);
    } catch (err) {
        console.warn('UUID resolution phase encountered an error:', err);
    }
