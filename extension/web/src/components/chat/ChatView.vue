    ...
    <div class="input">
      ...
      <div class="input-container">
        ...
        <div class="input-area">
          ...
          <!-- Draw Whole Circuit: sends message with action flag -->
          <IconButton
            class="draw-circuit-btn"
            icon="CircuitBoard"
            :size="16"
            :disabled="newMessage.trim() === '' || isLoading"
            title="Draw Whole Circuit — generate and place entire schematic"
            @click="sendDrawCircuit"
          />
...
/**
 * Sends the current message with action:"draw_circuit" flag so the backend
 * routes it through the full circuit-generation pipeline.
 */
function sendDrawCircuit() {
  if (!newMessage.value.trim() || isLoading.value) return;
  sendMessage({ action: 'draw_circuit' });
}
...
/* Draw Whole Circuit button */
.input-area .draw-circuit-btn {
  padding: 0.5rem;
  color: #7c3aed;
  background-color: transparent;
  border: none;
...
.input-area .draw-circuit-btn:hover:not(:disabled) {
  background-color: rgba(124, 58, 237, 0.1);
}

.input-area .draw-circuit-btn:disabled {
