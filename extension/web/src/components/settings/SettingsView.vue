    ...
    <div class="settings-view-container">
        <div class="settings-view">
            ...
            <div class="settings-section">
                ...
                <p class="section-description">Configure your local AI backend server (default: http://localhost:5120)</p>

                <div class="setting-group">
                    <label for="backendUrl">Backend URL</label>
                    <div class="input-with-btn">
                        <input id="backendUrl" :value="settings.backendUrl || 'http://localhost:5120'" type="text"
                            placeholder="http://localhost:5120"
                            @input="onSettingInput('backendUrl', ($event.target as HTMLInputElement).value)" />
                        <button @click="testConnection" class="test-conn-btn" :disabled="testingConnection">
                            {{ testingConnection ? 'Testing...' : 'Test Connection' }}
                        </button>
                    </div>
                    <p v-if="connTestResult" class="conn-result" :class="connTestResult.ok ? 'conn-ok' : 'conn-fail'">
                        {{ connTestResult.message }}
                    </p>
                    <p class="hint">URL of the local backend FastAPI server. Keep default unless you changed the port.</p>
                </div>
            </div>
...
const connTestResult = ref<{ ok: boolean; message: string } | null>(null);

async function testConnection() {
    testingConnection.value = true;
    connTestResult.value = null;

    const url = (settingsStore.getSetting('backendUrl') as string) || 'http://localhost:5120';

    try {
        const res = await fetch(url.replace(/\/$/, '') + '/api/health');
        if (res.ok) {
            const data = await res.json();
            const model = data.model || data.llm_model || 'unknown model';
            connTestResult.value = { ok: true, message: `Connected — model: ${model}` };
        } else {
            connTestResult.value = { ok: false, message: `Server returned HTTP ${res.status}` };
        }
    } catch (err) {
        connTestResult.value = {
            ok: false,
            message: `Cannot connect to ${url}: ${err instanceof Error ? err.message : 'Connection refused'}`
        };
    } finally {
